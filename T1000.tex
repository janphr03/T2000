...
\graphicspath{{figures/}} % Bilder in ./figures/ ablegen
...
\subsection{Entwurf} 
    ...
    Die DLL wurde in \texttt{.NET 8} umgesetzt. Zentrale Artefakte: \texttt{IConnector} (Abstraktion), 
    \texttt{MySqlAccess} (Implementierung), \texttt{ErrorValues} (Fehlerenum), Logging via 
    \texttt{ILogger<T>} (DI). Entscheidungen: (1) Fehlercodierung statt ungefilterter Exceptions, 
    (2) SQL-Strings zunächst direkt; parametrisierte Queries als Ausblick, 
    (3) Austauschbarkeit des DB-Providers durch Programmierung gegen das Interface.
    Werkzeuge: Visual Studio / \texttt{dotnet}, MySQL-Connector. 
    Herausforderung: sauberes Mapping typischer MySQL-Fehler → \texttt{AuthenticationFailed}, 
    \texttt{DatabaseNotFound}, \texttt{DuplicateEntry}. 
    Dieses Vorgehen entspricht den Empfehlungen zur Trennung von Schnittstellen und Implementierung 
    sowie zur Nutzung von Dependency Injection für flexible und testbare Architekturen 
    (vgl. \cite{martin_clean_2018}, Kap.~7–11; \cite{troelsen_pro_2017}, S.~509, 560).

    \subsection{Validierung}
    Unit-Tests (AAA) prüfen u.\,a. Verbindungsaufbau mit falschen Credentials (→ 
    \texttt{AuthenticationFailed}), idempotentes Schließen (\texttt{ConnectionAlreadyClosed}) 
    und Schreiboperationen ohne betroffene Zeilen (\texttt{NoData}). 
    Logging wurde über Mocks isoliert. Geplante Integrationstests: MySQL in Docker für 
    End-to-End-Verhalten. Ergebnis: definierte Fehlerpfade und Statusrückgaben verhalten sich 
    wie spezifiziert; verbleibende Risiken betreffen parametrisierte Queries und Transaktionen (Ausblick).
    Die gewählte Teststrategie orientiert sich an etablierten Prinzipien für Softwaretests 
    (vgl. \cite{sommerville_software_2012}, Kap.~20).
...
%===================================================================================
    ...
    (z.B. \texttt{errorValues.ConnectionFailed}, \texttt{errorValues.NoData}, \texttt{errorValues.DuplicateEntry}).
...
\end{itemize}
    ...
    Wie in Abbildung~\ref{fig:uml-connector} gezeigt, stellen die Interfaces eine Schnittstelle zur Verfügung 
    die beliebig erweiterbar und in der Anwendung generisch zu verwenden ist.

    \begin{itemize}
  \item \texttt{IConnector.cs}: Definiert die vertraglichen Methoden für den Datenbankzugriff.
  \item \texttt{MySqlAccess.cs}: Konkrete Implementierung von \texttt{IConnector} für \acrshort{mysql}.
  \item \texttt{ErrorValues.cs}: Zentrales \texttt{enum} \texttt{ErrorValues} für Status- und Fehlerzustände.
\end{itemize}
    ...
    \includegraphics[
        page=1,
        width=\textwidth,            % maximale Breite
        height=.9\textheight,        % und hoch ausreizen
        keepaspectratio,             % Seitenverhältnis behalten
        trim=30mm 8mm 45mm 70mm,      % L U R O (links, unten, rechts, oben)
        clip
    ]{UMLKlassendiagrammDbConnector.pdf}
    \caption{Klassendiagramm der Verbindungsbibliothek (\texttt{App}, \texttt{IConnector}, \texttt{MySqlAccess}, \texttt{ILogger}, \texttt{ErrorValues}).}
    \label{fig:uml-connector}
    \end{figure}
    ...
    %==========================HIER NOCH AUSFÜHRLICHER SCHREIBEN=================================
    \subsection{Implementierung}

    \textbf{Zentrale Klassen und Methoden:}

    Die Klasse \texttt{MySqlAccess} bildet den Kern der Verbindungsbibliothek. Sie kapselt den gesamten Lebenszyklus einer Datenbankverbindung und stellt Methoden für die grundlegenden CRUD-Operationen bereit.  
    Jede dieser Methoden folgt demselben Ablaufmuster:  
    \begin{enumerate}
        \item Validierung der Eingabeparameter,
        \item Konstruktion und Ausführung des SQL-Befehls,
        \item Differenzierte Fehlerbehandlung,
        \item Rückgabe eines standardisierten Statuswertes.
    \end{enumerate}

    \begin{itemize}

        \item \texttt{openConnection()}:  
        Diese Methode ist für den Aufbau der Datenbankverbindung verantwortlich.  
        Vor dem eigentlichen Verbindungsaufbau wird zunächst geprüft, ob bereits eine aktive Verbindung besteht. 
        Auf diese Weise werden doppelte Öffnungen vermieden, die sonst zu Ressourcenlecks oder widersprüchlichen Zuständen führen könnten.  
        Schlägt der Aufbau fehl, was durch falsche Zugangsdaten, eine nicht existierende Datenbank oder einen nicht erreichbaren Server ausgelöst werden kann, 
        wird der Status \texttt{errorValues.AuthenticationFailed}, \texttt{errorValues.DatabaseNotFound}, oder \texttt{errorValues.ServerConnectionFailed} zurückgegeben. 
        Tritt ein allgemeiner Fehler beim Verbindungsaufbau auf, wird \texttt{errorValues.ConnectionQueryError} gesetzt.  
        Gleichzeitig wird der Fehler im Logger dokumentiert, sodass die Ursache später nachvollziehbar bleibt. 

        \item \texttt{closeConnection()}:  

        Das kontrollierte Schließen einer aktiven Verbindung ist ebenso wichtig wie der Verbindungsaufbau.  
        In dieser Methode wird geprüft, ob die Verbindung tatsächlich geöffnet ist. 
        Liegt bereits ein geschlossener Zustand vor, löst die Bibliothek keine Ausnahme aus, sondern liefert den klar definierten Status \texttt{errorValues.ConnectionAlreadyClosed}.  
        Dadurch können Aufrufer auch bei mehrfachen Schließversuchen stabil weiterarbeiten, was vor allem in größeren Anwendungen mit komplexen Steuerungsflüssen relevant ist. 
        Neben der Robustheit spielt hier auch die Ressourcenschonung eine Rolle, da Datenbankverbindungen stets aktiv Systemressourcen belegen.

        \item \texttt{select(\dots)}:  
        Für Leseoperationen wird ein \texttt{MySqlCommand} mit dem übergebenen SQL-Statement erstellt und ein \texttt{MySqlDataReader} geöffnet.  
        Dieser Reader ermöglicht ein effizientes sequentielles Auslesen der Ergebnismenge direkt vom Server.  
        Anschließend werden die Daten in ein \texttt{DataTable}-Objekt geladen, das an den Aufrufer zurückgegeben wird.  
        Dieses Vorgehen verbindet die Performancevorteile des Readers mit dem Komfort einer tabellarischen Datenstruktur, die sich leicht weiterverarbeiten, filtern oder serialisieren lässt.  
        Eine Konsolenausgabe innerhalb der Bibliothek erfolgt nicht. Die Darstellung bleibt bewusst der Anwendungsschicht überlassen, um die Trennung von Logik und Präsentation zu wahren.

        \item \texttt{insert(\dots)}, \texttt{update(\dots)}, \texttt{delete(\dots)}:  
        Die schreibenden Operationen folgen einem einheitlichen Muster: Nach Validierung der Eingabeparameter wird der SQL-Befehl konstruiert und mit \texttt{ExecuteNonQuery()} ausgeführt.  
        Das Ergebnis dieser Ausführung wird über die Anzahl betroffener Zeilen bestimmt.  
        Wird mindestens eine Zeile verändert, gibt die Methode den Status \texttt{Success} zurück. Bleibt die Anzahl bei null, so signalisiert die Bibliothek \texttt{NoData}.  
        Bei Verstößen gegen Integritätsbedingungen, etwa durch doppelte Primärschlüssel, erfolgt eine gezielte Abbildung auf \texttt{errorValues.DuplicateEntry}.  
        Damit wird nicht nur sichergestellt, dass Fehler kontrolliert behandelt werden, sondern auch, dass Anwendungen klar unterscheiden können, ob ein Befehl erfolgreich war, keine Wirkung hatte oder aufgrund einer Datenbankregel scheiterte.
    ...
    Ein zentrales Designziel der Verbindungsbibliothek ist die robuste und differenzierte Behandlung typischer Fehlerfälle. 
    Während der Standard-MySQL-Connector in \csharp{} im Fehlerfall rohe \texttt{MySqlException}s wirft, 
    kapselt die Bibliothek diese intern und übersetzt sie in wohldefinierte Statuswerte des Enums \texttt{errorValues}. 
    Dadurch entfällt für die aufrufende Anwendung die Notwendigkeit, Datenbank-spezifische Exceptions im Detail auswerten zu müssen. 
    Stattdessen erhält der Entwickler eine konsistente und klar interpretierbare Rückmeldung, die unabhängig von der zugrunde liegenden Datenbank-Engine funktioniert. 

    Besonders relevant ist dies bei häufig auftretenden Fehlern, die im Produktivbetrieb immer wieder auftreten können:  

    \begin{itemize}
        \item Fehler \texttt{errorValues.AuthenticationFailed (-6)}: tritt auf, wenn falsche Zugangsdaten wie Benutzername oder Passwort übergeben wurden.  
        Dies erlaubt es der Anwendung, gezielt darauf zu reagieren, etwa durch das erneute Anfordern korrekter Credentials oder eine Benutzerbenachrichtigung.

        \item Fehler \texttt{errorValues.DatabaseNotFound (-7)}: signalisiert, dass die in der Verbindungszeichenkette angegebene Datenbank nicht existiert.  
        Statt einen Abbruch durch eine Exception zu riskieren, meldet die Bibliothek diesen Zustand über \texttt{errorValues.DatabaseNotFound}.  
        So kann die Anwendung etwa automatisiert eine Fallback-Strategie ausführen oder die initiale Einrichtung einer neuen Datenbank anstoßen.

        \item Fehler \texttt{errorValues.DuplicateEntry (-13)}: entsteht bei Verletzungen von Eindeutigkeits- oder Primärschlüssel-Constraints, beispielsweise beim Einfügen eines Datensatzes mit bereits existierender ID.  
        Die Bibliothek differenziert diesen Fall klar und gibt \texttt{errorValues.DuplicateEntry} zurück.  
        Auf diese Weise können Anwendungen gezielt Benutzerfeedback geben oder alternative Insert-Strategien (z.\,B. \texttt{ON DUPLICATE KEY UPDATE}) einsetzen.
    \end{itemize}

    Für alle weiteren, nicht explizit abgebildeten Fehler wird der generische Status \texttt{errorValues.UnknownError} zurückgegeben.  
    Damit wird sichergestellt, dass auch unerwartete Ausnahmefälle nicht unkontrolliert an die Anwendung durchgereicht werden und diese in einen instabilen Zustand versetzen. 
    Das Mapping erfolgt zentral in der Hilfsmethode \texttt{MapMySqlError}, sodass eine einheitliche Behandlung in allen CRUD-Operationen gewährleistet ist.  

    Dieser Ansatz bietet mehrere Vorteile:  
    \begin{itemize}
        \item \textbf{Konsistenz:} Alle Methoden geben Statuswerte im gleichen Format zurück, unabhängig davon, welcher konkrete MySQL-Fehler aufgetreten ist.  
        \item \textbf{Robustheit:} Anwendungslogik muss keine rohen Exceptions auswerten, wodurch Laufzeitfehler reduziert werden.  
        \item \textbf{Erweiterbarkeit:} Neue Fehlernummern können leicht ergänzt und auf zusätzliche Statuswerte gemappt werden, ohne dass Änderungen in der Anwendungslogik erforderlich sind.  
        \item \textbf{Transparenz:} Durch die Kombination mit dem Logger bleibt die konkrete Ursache dennoch nachvollziehbar (z.\,B. durch Speichern von Fehlernummer und Fehlermeldung).
    \end{itemize}

    Damit verbindet die Verbindungsbibliothek eine für den Entwickler einfach nutzbare Oberfläche mit der notwendigen technischen Tiefe, um reale Betriebsfehler differenziert zu behandeln und kontrolliert weiterzugeben.
    ...
    // Test: openConnection() liefert AuthenticationFailed bei falschen Credentials
    [Fact]
    public void OpenConnection_ReturnsAuthenticationFailed_WhenCredentialsAreInvalid()
    {
        ...
        string uid = "invalidUser"; // absichtlich falscher Benutzer
        ...
        // Assert
        Assert.Equal(errorValues.AuthenticationFailed, result);
    }
    \end{lstlisting}
    ...
    \subsection{Motivation und Rahmenbedingungen}
    Die Fertigung beschreibt an einer festen Speicheradresse für jede LED einen numerischen Code, 
    der definiert, ob die LED ausgeschaltet, rot oder grün leuchten soll. 
    Meine Aufgabe bestand darin, diese Werte auszulesen und mit den bestehenden API-Funktionen 
    (\texttt{ledHandler\_set()}) auf die Hardware abzubilden. 
    Die dafür notwendigen Datentypen und Schnittstellen wurden bereits im Projekt von der 
    Softwarearchitektur bereitgestellt. 
    Die Eigenleistung liegt in der Kombination dieser Bausteine zu einer 
    kohärenten Methode.

    \subsection{Datenhaltung im Speicher}
    Damit die Fertigung überhaupt Werte hinterlegen kann, wird ein spezieller Speicherbereich 
    im Flash als zweidimensionales Array reserviert.  
    Jede Zeile des Arrays repräsentiert eine LED-Klasse (z. B. \texttt{pin1}, \texttt{pin2}, 
    Versorgungsspannung oder Corner-LED), jede Spalte eine konkrete Instanz.  
    So entsteht eine Matrix aus fünf Zeilen und acht Spalten, die den Zustand aller LEDs abbildet.  

    \begin{lstlisting}[language=C]
    #define ROWS 5
    #define COLLS 8

    uint8_t colors_from_pins_arr[ROWS][COLLS] 
        __attribute__((section(".myNewSection")));
    \end{lstlisting}

    \textbf{Hintergedanke:} Die Fertigung muss nur an einer bekannten Speicherstelle schreiben, 
    die Firmware liest später an genau dieser Stelle die Werte aus. 
    So bleibt die Schnittstelle zwischen Hardware und Software eindeutig.

    \subsection{Initialisierung}
    Beim Start des Systems wird das Array mit Nullen belegt. 
    Das stellt sicher, dass keine undefinierten Werte vorliegen, falls die Fertigung 
    den Speicherbereich noch nicht beschrieben hat.  

    \begin{lstlisting}[language=C]
    void initColors(void) {
        memset(colors_from_pins_arr, 0, sizeof(colors_from_pins_arr));
    }
    \end{lstlisting}

    \textbf{Hintergedanke:} Ein deterministischer Startzustand verhindert unvorhersehbares Verhalten 
    (z. B. zufälliges Aufleuchten von LEDs).

    \subsection{Abbildung von LED-Zuständen}
    Um die Werte aus dem Array in konkrete LED-Zustände übersetzen zu können, 
    wird eine Struktur definiert, die die drei zentralen Parameter einer LED zusammenfasst:  
    \begin{itemize}
    \item die \textbf{Klasse} der LED (welcher Pin sie repräsentiert),  
    \item das \textbf{Verhalten} (aus, konstant, blinken),  
    \item die \textbf{Farbe} (rot, grün, gelb).  
    \end{itemize}

    \begin{lstlisting}[language=C]
    typedef struct {
        ledHandler_ledClass_t pin_ID;
        ledHandler_behavior_t behavior;
        ledHandler_color_t color;
    } pin_state_t;
    \end{lstlisting}

    \textbf{Hintergedanke:} Durch diese Struktur kann der Zustand jeder LED logisch zusammengefasst 
    werden, ohne dass man an mehreren Stellen im Code dieselben Parameter handhaben muss.

    \subsection{Die Mapping-Funktion \texttt{evaluate\_led\_color()}}
    Der zentrale Bestandteil ist die Funktion, die alle Werte durchläuft, interpretiert 
    und über die API an die LEDs weitergibt.  

    1. Zunächst wird für jede Zeile (also jede LED-Klasse) bestimmt, 
    welche Enumeration (\texttt{ledHandler\_ledClass\_t}) zugeordnet ist.  
    2. Danach werden alle Instanzen dieser Klasse durchlaufen.  
    3. Der gespeicherte Wert (0, 1, 2, …) wird ausgelesen und in ein Verhalten sowie eine Farbe übersetzt.  
    \begin{itemize}
        \item 0 = LED aus  
        \item 1 = LED rot, konstant  
        \item 2 = LED grün, konstant  
        \item weitere Werte können später für Blinkmuster oder zusätzliche Farben reserviert werden  
    \end{itemize}
    4. Abschließend wird die Funktion \texttt{ledHandler\_set()} aufgerufen, 
    die bereits von der Architektur bereitgestellt wurde.  

    \begin{lstlisting}[language=C]
    void evaluate_led_color(void){
        for(uint8_t row = 0; row < ROWS; row++){
            ledHandler_ledClass_t current_pin = ...; // Klasse bestimmen

            for(uint8_t inst = 0; inst < COLLS; inst++){
                uint8_t code = colors_from_pins_arr[row][inst];

                if(code == 0){
                    behavior = ledHandler_behavior_off;
                    color    = ledHandler_color_red;
                } else if(code == 1){
                    behavior = ledHandler_behavior_solid;
                    color    = ledHandler_color_red;
                } else if(code == 2){
                    behavior = ledHandler_behavior_solid;
                    color    = ledHandler_color_green;
                } else {
                    behavior = ledHandler_behavior_solid;
                    color    = ledHandler_color_yellow;
                }

                ledHandler_set(current_pin, inst, behavior, color);
            }
        }
    }
    \end{lstlisting}

    \textbf{Hintergedanke:} Die Logik bildet eine Brücke zwischen den von der Fertigung 
    hinterlegten Rohdaten (Zahlenwerte) und der abstrakten LED-API. 
    Die Implementierung ist bewusst generisch, damit spätere Erweiterungen (weitere Farben, Blinkmuster) 
    leicht ergänzt werden können.

    \subsection{Zusammenfassung}
    Damit konnte eine Funktion entwickelt werden, die die Schnittstelle zwischen Fertigungsdaten 
    und Hardware-Ansteuerung herstellt. Die Architektur (API und Datentypen) blieb dabei unverändert, 
    die Eigenleistung liegt in der methodischen Kombination und der Umsetzung der Mapping-Logik.
